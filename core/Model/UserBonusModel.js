...
class UserBonusModel{
    ...
    async addRegBonus(userId,referral_path){
        try {
            ...
            let isEnableReferral = regConfig.find((item)=>{return item.config_key == 'isEnableReferral'}).config_value == '1' ? true : false;
            let referralLevel = parseInt(regConfig.find((item)=>{return item.config_key == 'referralLevel'}).config_value);
            let coinId = parseInt(regConfig.find((item)=>{return item.config_key == 'coinId'}).config_value);
            let L0Amount = parseFloat(regConfig.find((item)=>{return item.config_key == 'L0Amount'}).config_value);
            let L1Amount = parseFloat(regConfig.find((item)=>{return item.config_key == 'L1Amount'}).config_value);
            let L2Amount = parseFloat(regConfig.find((item)=>{return item.config_key == 'L2Amount'}).config_value);
            let L3Amount = parseFloat(regConfig.find((item)=>{return item.config_key == 'L3Amount'}).config_value);
